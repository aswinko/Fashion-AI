...
const upsertProductRecord = async (product: Stripe.Product) => {
  ...
  const manageSubscriptionStatusChange = async (
    ...
    console.log(subscription.items.data[0].current_period_start);
  ...
  const updateUserCredits = async (userId: string, metadata: Json) => {
    const creditData: TablesInsert<"credits"> = {
      user_id: userId,
   ...
   const {error: upsertError} =  await supabaseAdmin.from("credits").upsert(creditData).eq('user_id', userId)
